...
void cannonAttack(pair<int, int> attacker, pair<int, int> target){
    ...
    for(int i = 0; i < 8; ++i){
        int splash_row = (target.first+splash_r[i])%(N+1) ? (target.first+splash_r[i])%(N+1) : 1;
        int splash_col = (target.second+splash_c[i])%(M+1) ? (target.second+splash_c[i])%(M+1) : 1;
...
bool laserAttack(pair<int, int> attacker, pair<int, int> target){
    ...
    while(!q.empty()){
        ...
        for(int i = 0; i < 4; ++i){
            pair<int, int> next;
            next.first = (cur.first + dr[i]) % (N+1) ? (cur.first + dr[i]) % (N+1) : 1;
            next.second = (cur.second + dc[i]) % (M+1) ? (cur.second + dc[i]) % (M+1) : 1;
